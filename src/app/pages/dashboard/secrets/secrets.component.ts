...
@Component({
    ...
    <div class="page-container">
      ...
      <div class="grid">
        ...
        <div class="card tokens-card">
          ...
          <p class="section-desc">These tokens are securely stored in the CDD Control Plane Vault.</p>
          ...
          <form [formGroup]="tokensForm" (ngSubmit)="onSaveTokens()">
            ...
            <div class="form-actions">
              <button type="submit" class="btn btn-primary" [disabled]="tokensForm.pristine || tokensState().status === 'loading'">
        ...
        <div class="card actions-card">
          ...
          <p class="section-desc">Manually initiate generation or publishing for linked repositories.</p>
          ...
          <div class="action-item">
            ...
            <button class="btn btn-secondary" (click)="triggerAction('generate')" [disabled]="generateState().status === 'loading'">
              {{ generateState().status === 'loading' ? 'Running...' : 'Run Generation' }}
            </button>
          </div>
          @if (generateState().message) {
            <div class="alert" [class.alert-success]="generateState().status === 'success'" [class.alert-error]="generateState().status === 'error'">
          ...
          <div class="action-item">
            ...
            <button class="btn btn-secondary" (click)="triggerAction('publish')" [disabled]="publishState().status === 'loading'">
              {{ publishState().status === 'loading' ? 'Running...' : 'Run Publishing' }}
            </button>
          </div>
          @if (publishState().message) {
            <div class="alert" [class.alert-success]="publishState().status === 'success'" [class.alert-error]="publishState().status === 'error'">
  ...
  styles: `
    ...
    .btn-primary {
      ...
      &:hover:not(:disabled) { background-color: #2c974b; }
    ...
    .action-info {
      ...
      p { margin: 0; font-size: 0.875rem; color: var(--color-text-muted, #57606a); }
    }
    .divider {
      border: 0;
    ...
    .alert-success { background: #dcffe4; color: #2da44e; border: 1px solid #2da44e; }
    .alert-error { background: #ffebe9; color: #cf222e; border: 1px solid #cf222e; }
  `
})
export class SecretsComponent {
  ...
  triggerAction(action: 'generate' | 'publish'): void {
    ...
    setTimeout(() => {
      state.set({ status: 'success', message: `${action.charAt(0).toUpperCase() + action.slice(1)} completed successfully.` });
