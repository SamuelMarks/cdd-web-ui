...
import { selectOrientation, selectSelectedLanguageId, selectOpenApiSpecContent, selectTarget, selectCurrentLanguageOptions, selectOpenApiInputFormat } from './selectors';
  ...
  executeRun$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([_, orientation, languageId, specContent, target, languageOptions, inputFormat]) => {
        ...
        if (!lang) {
          return of(WorkspaceActions.executeRunFailure({ error: `Selected language ID '${languageId}' not found.` }));
        }

        if (!lang.availableInWasm) {
           return of(WorkspaceActions.executeRunFailure({ error: `Language '${lang.name}' does not support offline WASM generation.` }));
        }
        ...
        if (orientation === 'openapi-left') {
          ...
          return upgradePromise.then(upgradedSpec => 
            ...
            (error) => WorkspaceActions.executeRunFailure({ error: error instanceof Error ? error.message : String(error) })
          );
        } else {
          // Direction: to_openapi -> Spec Generation (Not fully implemented in WASM SDK yet, placeholder)
          return Promise.resolve(WorkspaceActions.executeRunFailure({ error: 'Generating OpenAPI from Code (to_openapi) is not yet supported in WASM offline mode.' }));
        }
      })
    )
  );
