import { workspaceReducer, fileTreeReducer, openApiReducer, initialWorkspaceState, initialFileTreeState, initialOpenApiState } from './reducers';
...
describe('Reducers', () => {
  describe('WorkspaceReducer', () => {
    ...
    it('should set orientation explicitly', () => {
      const state = workspaceReducer(initialWorkspaceState, Actions.setOrientation({ orientation: 'openapi-right' }));
      expect(state.orientation).toBe('openapi-right');
    });

    it('should set selected language', () => {
      const state = workspaceReducer(initialWorkspaceState, Actions.setSelectedLanguage({ languageId: 'python' }));
      expect(state.selectedLanguageId).toBe('python');
    });

    it('should set target', () => {
      const state = workspaceReducer(initialWorkspaceState, Actions.setTarget({ target: 'to_server' }));
      expect(state.target).toBe('to_server');
    });

    it('should set language options', () => {
      const state = workspaceReducer(initialWorkspaceState, Actions.setLanguageOptions({ languageId: 'typescript', options: { framework: 'fetch' } }));
  ...
  describe('FileTreeReducer', () => {
    ...
    it('should clear active file path if no files are generated', () => {
      const state = fileTreeReducer({ ...initialFileTreeState, activeFilePath: 'old.ts' }, Actions.setGeneratedFiles({ files: [] }));
      expect(state.files).toEqual([]);
      expect(state.activeFilePath).toBeNull();
    });

    it('should select a file', () => {
      const state = fileTreeReducer(initialFileTreeState, Actions.selectFile({ filePath: 'new.ts' }));
  ...
  describe('OpenApiReducer', () => {
    it('should update open api spec content', () => {
      const state = openApiReducer(initialOpenApiState, Actions.updateOpenApiSpec({ content: 'new content' }));
      expect(state.specContent).toBe('new content');
    });

    it('should set open api validation errors', () => {
      const state = openApiReducer(initialOpenApiState, Actions.setOpenApiValidationErrors({ errors: ['error1', 'error2'] }));
      expect(state.validationErrors).toEqual(['error1', 'error2']);
    });

    it('should set open api input format', () => {
      const state = openApiReducer(initialOpenApiState, Actions.setInputFormat({ format: 'google_discovery' }));
